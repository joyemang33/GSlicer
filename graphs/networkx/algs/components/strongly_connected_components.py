...
class StronglyConnectedComponentsOracle:
    def __init__(self):
    ...
    def validate_oracle(self):
        scc_G = list(nx.strongly_connected_components(self.G))
        
        scc_G1 = list(nx.strongly_connected_components(self.G1)) if len(self.G1) > 0 else []
        scc_G2 = list(nx.strongly_connected_components(self.G2)) if len(self.G2) > 0 else []
        
        violations = []
        
        for i, component in enumerate(scc_G1):
            parent_components = []
            for j, scc in enumerate(scc_G):
                if component.issubset(scc):
            ...
            if len(parent_components) == 0:
                violations.append({
                    ...
                    'error': 'Not a subset of any strongly connected component in G'
                })
        
        for i, component in enumerate(scc_G2):
            parent_components = []
            for j, scc in enumerate(scc_G):
                if component.issubset(scc):
                    parent_components.append(j)
        ...
        if violations:
            return False, {
                ...
                'scc_G1': scc_G1,
                'scc_G2': scc_G2,
                'num_scc_G': nx.number_strongly_connected_components(self.G),
                'num_scc_G1': nx.number_strongly_connected_components(self.G1) if len(self.G1) > 0 else 0,
                'num_scc_G2': nx.number_strongly_connected_components(self.G2) if len(self.G2) > 0 else 0
            }
    ...
    def test(self):
        ...
        if not result:
            ...
            print(f"  G: {details['num_scc_G']}")
            print(f"  G1: {details['num_scc_G1']}")
            print(f"  G2: {details['num_scc_G2']}")
            ...
            for i, scc in enumerate(details['scc_G']):
                print(f"  Component {i}: {scc}")
            
            print(f"\nStrongly connected components in {violation['subgraph']}:")
            scc_list = details['scc_G1'] if violation['subgraph'] == 'G1' else details['scc_G2']
            for i, scc in enumerate(scc_list):
                print(f"  Component {i}: {scc}")
            
            print(f"\nIs G strongly connected? {nx.is_strongly_connected(self.G)}")
            if len(self.G1) > 0:
                print(f"Is G1 strongly connected? {nx.is_strongly_connected(self.G1)}")
            if len(self.G2) > 0:
                print(f"Is G2 strongly connected? {nx.is_strongly_connected(self.G2)}")
...
if __name__ == "__main__":
    random.seed(42)
    oracle_tester = StronglyConnectedComponentsOracle()
