...
class BiconnectedComponentsOracle:
    def __init__(self):
    ...
    def validate_oracle(self):
        bcc_G = list(nx.biconnected_components(self.G))
        
        bcc_G1 = list(nx.biconnected_components(self.G1)) if len(self.G1) > 0 else []
        bcc_G2 = list(nx.biconnected_components(self.G2)) if len(self.G2) > 0 else []
        
        violations = []
        
        for i, component in enumerate(bcc_G1):
            parent_components = []
            for j, bcc in enumerate(bcc_G):
                if component.issubset(bcc):
            ...
            if len(parent_components) == 0:
                violations.append({
                    ...
                    'error': 'Not a subset of any biconnected component in G'
                })
        
        for i, component in enumerate(bcc_G2):
            parent_components = []
            for j, bcc in enumerate(bcc_G):
                if component.issubset(bcc):
                    parent_components.append(j)
        ...
        if violations:
            return False, {
                ...
                'bcc_G2': bcc_G2,
                'num_bcc_G': len(bcc_G),
                'num_bcc_G1': len(bcc_G1),
                'num_bcc_G2': len(bcc_G2),
                'articulation_points_G': list(nx.articulation_points(self.G)),
                'articulation_points_G1': list(nx.articulation_points(self.G1)) if len(self.G1) > 0 else [],
                'articulation_points_G2': list(nx.articulation_points(self.G2)) if len(self.G2) > 0 else []
            }
    ...
    def test(self):
        ...
        if not result:
            ...
            print(f"  G: {details['num_bcc_G']}")
            print(f"  G1: {details['num_bcc_G1']}")
            print(f"  G2: {details['num_bcc_G2']}")
            
            print(f"\nArticulation points:")
            print(f"  G: {details['articulation_points_G']}")
            print(f"  G1: {details['articulation_points_G1']}")
            print(f"  G2: {details['articulation_points_G2']}")
            ...
            print("\nBiconnected components in G:")
            for i, bcc in enumerate(details['bcc_G']):
                print(f"  Component {i}: {bcc}")
            
            print(f"\nBiconnected components in {violation['subgraph']}:")
            bcc_list = details['bcc_G1'] if violation['subgraph'] == 'G1' else details['bcc_G2']
            for i, bcc in enumerate(bcc_list):
                print(f"  Component {i}: {bcc}")
            
            print(f"\nIs G biconnected? {nx.is_biconnected(self.G)}")
            if len(self.G1) > 0:
                print(f"Is G1 biconnected? {nx.is_biconnected(self.G1)}")
            if len(self.G2) > 0:
                print(f"Is G2 biconnected? {nx.is_biconnected(self.G2)}")
...
if __name__ == "__main__":
    random.seed(42)
    oracle_tester = BiconnectedComponentsOracle()
