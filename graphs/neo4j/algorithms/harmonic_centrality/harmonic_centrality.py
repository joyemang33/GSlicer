...
class TestNeo4jKcore(Neo4jTester):
    ...
    def validate(self):
        ...
        res_G = self.gds.alpha.closeness.harmonic.stream(self.G).to_dict('records')
        res_G0 = self.gds.alpha.closeness.harmonic.stream(self.G0).to_dict('records')
        res_G1 = self.gds.alpha.closeness.harmonic.stream(self.G1).to_dict('records')
        d_G, d_G0, d_G1 = dict(), dict(), dict()
        for x in res_G: d_G[x["nodeId"]] = x["centrality"] * (len(res_G) - 1)
        for x in res_G0: d_G0[x["nodeId"]] = x["centrality"] * (len(res_G0) - 1)
        for x in res_G1: d_G1[x["nodeId"]] = x["centrality"] * (len(res_G1) - 1)
